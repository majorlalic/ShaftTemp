...
CREATE TABLE alarm (
    ...
    merge_key varchar(128) COMMENT '待确认合并占位键',
    status tinyint COMMENT '告警状态码 0待确认1已确认2持续观察3误报4关闭5自动恢复',
    first_alarm_time datetime COMMENT '首次告警时间',
    last_alarm_time datetime COMMENT '最近告警时间',
    merge_count int unsigned COMMENT '合并次数',
    event_count int unsigned COMMENT '已记录事件次数',
...
CREATE TABLE event (
    ...
    partition_no int COMMENT '分区序号',
    source_format varchar(30) COMMENT '数据来源格式',
    event_type tinyint COMMENT '事件类型码 0触发1合并2确认3观察4误报5恢复6关闭',
    ...
    content varchar(1000) COMMENT '事件内容',
