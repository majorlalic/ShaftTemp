...
CREATE TABLE alarm_rule (
    ...
    scope_type varchar(20) DEFAULT NULL COMMENT '当前固定为 GLOBAL',
    scope_id bigint unsigned DEFAULT NULL COMMENT '当前固定为 NULL',
...
当前项目已确认所有规则均为全局生效，因此：

- `scope_type` 固定为 `GLOBAL`
- `scope_id` 固定为 `NULL`

这两个字段暂时保留，只是为了兼容当前库结构和接口，不参与差异化匹配。
