...
public interface DeviceRepository {
    ...
    @Select({
        ...
        "left join ODS_DWEQ_DM_AREA_D a on a.id = d.area_id",
        ...
        "</if>",
        "<if test='areaId != null'>",
        "and (d.area_id = #{areaId} or instr('/' || nvl(a.path_ids, '') || '/', '/' || #{areaId} || '/') &gt; 0)",
        "</if>",
    ...
    @Select({
        ...
        "    d.asset_status, d.area_id, d.org_id, a.name as area_name, d.online_status, d.last_report_time, d.last_offline_time,",
        "    d.remark, d.created_on, d.updated_on,",
        "    b.monitor_id as monitor_id,",
        "    m.name as monitor_name,",
        "    row_number() over(order by case when d.updated_on is null then 1 else 0 end, d.updated_on desc, d.id desc) rn",
        "  from ODS_DWEQ_DM_DEVICE_D d",
        "  left join ODS_DWEQ_DM_AREA_D a on a.id = d.area_id",
